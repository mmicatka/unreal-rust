...
void FDynamicRustComponent::Render(TSharedRef<IPropertyHandle> MapHandle, IDetailCategoryBuilder &DetailBuilder,
  ...
  for (uint32 ComponentIdx = 0; ComponentIdx < NumberOfComponents; ++ComponentIdx) {
	...
	ComponentGroup.HeaderRow().NameContent()[FieldsProperty->CreatePropertyNameWidget(FText::FromString(Name))].ExtensionContent()[SNew(SBox)
																																	   .HAlign(HAlign_Center)
																																	   .VAlign(VAlign_Center)
																																	   .WidthOverride(22)
																																	   .HeightOverride(22)[SNew(SButton)
	...
	for (uint32 FieldIdx = 0; FieldIdx < NumberOfFields; ++FieldIdx) {
	  ...
	  if (Tag == ERustPropertyTag::Float) {
		...
		ComponentGroup.AddPropertyRow(FloatProperty.ToSharedRef()).DisplayName(FText::FromString(FieldPropertyName));
	  }
	  if (Tag == ERustPropertyTag::Vector) {
		...
		ComponentGroup.AddPropertyRow(VectorProperty.ToSharedRef()).DisplayName(FText::FromString(FieldPropertyName));
	  }
	  if (Tag == ERustPropertyTag::Bool) {
		auto BoolProperty = RustPropertyEntry->GetChildHandle(
			GET_MEMBER_NAME_CHECKED(FRustProperty, Bool));
		ComponentGroup.AddPropertyRow(BoolProperty.ToSharedRef()).DisplayName(FText::FromString(FieldPropertyName));
	  }
	  if (Tag == ERustPropertyTag::Quat) {
		...
		ComponentGroup.AddPropertyRow(QuatProperty.ToSharedRef()).DisplayName(FText::FromString(FieldPropertyName));
	  }
	  if (Tag == ERustPropertyTag::Class) {
		...
		ComponentGroup.AddPropertyRow(ClassProperty.ToSharedRef()).DisplayName(FText::FromString(FieldPropertyName));
	  }
	  if (Tag == ERustPropertyTag::Sound) {
		...
		ComponentGroup.AddPropertyRow(SoundProperty.ToSharedRef()).DisplayName(FText::FromString(FieldPropertyName));
	  }
	}
  }
}
